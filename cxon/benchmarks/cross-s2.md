### `CXON` benchmarks / `cross-compiler`

--------------------------------------------------------------------------------

###### Read
  ![read / native (default)](figures/cross.head.default.json.native-s2-read.svg)
  ![read / native (boost::charconv)](figures/cross.head.boost-charconv.json.native-s2-read.svg)
  ![read / node (default)](figures/cross.head.default.json.node-s2-read.svg)
  ![read / node (boost::charconv)](figures/cross.head.boost-charconv.json.node-s2-read.svg)

###### Write
  ![write / native (default)](figures/cross.head.default.json.native-s2-write.svg)
  ![write / native (boost::charconv)](figures/cross.head.boost-charconv.json.native-s2-write.svg)
  ![write / node (default)](figures/cross.head.default.json.node-s2-write.svg)
  ![write / node (boost::charconv)](figures/cross.head.boost-charconv.json.node-s2-write.svg)
